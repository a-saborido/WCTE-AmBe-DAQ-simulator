...
This component builds candidate-level neutron-tagging inputs for WCTE AmBe
...
repository root, first run `cd tagging-models`.
...
data/
  wcte_ambe_mc_plus_clean_bkg_pe_*_TRAIN.root
...
candidates_extraction/
  ...
  config.py                   Input samples, source positions, extraction settings
...
The input ROOT files and their prompt/source positions are paired in
`INPUT_SAMPLES` in `candidates_extraction/config.py`. The same file also contains prompt
cuts, delayed-search timing, Nn/Nmax200 settings, double-grid multilateration
...
By default, extraction processes every `(ROOT file, prompt position)` pair in
`candidates_extraction/config.py` and combines their candidates in one table. Positions in
that configuration are in centimetres. For a one-off single-file run, use
`--root FILE --prompt-vertex-cm X Y Z`.
...
For the equivalent Slurm workflow, submit from this component directory so
Slurm resolves its log paths correctly:
...
These defaults live in `candidates_extraction/config.py` and are exposed as CLI options. Truth
...
- `source_file`, `source_file_id`, `source_entry`: input file and anchor readout
  window used to find the candidate.
- `event_number`, `prompt_id`, `candidate_id`: candidate identifiers inside that
  source window.
...
- the observable columns listed in `candidates_extraction/config.py`: all values available
  for model training.
...
default split strategy is `per_file_anchor_window`: for every source file, 50%
of its candidate-producing anchor windows go to training, 25% to validation,
and 25% to testing. All candidates from one anchor window stay together. The
plots are diagnostics:
...
The test suite uses Python's standard `unittest` framework, so pytest is not
required. Activate an environment containing the tagging-model dependencies,
then run from this `tagging-models/` directory:

```bash
python -m unittest discover -s tests -v
```

Discovery loads files named `test*.py` and runs their `unittest.TestCase`
methods. The tests cover BDT splitting and prediction, observable plots,
geometry and BONSAI-derived observables, continuous-window truth metadata, and
the multilateration vertex refit. They use synthetic inputs, temporary output
directories, and the checked-in text geofile; they do not require the large
ROOT samples. A successful run ends with `OK` and returns exit code zero.

## Notes For Development

- Add future observable code under `candidates_extraction/observables/` and expose it
  through the observable groups in `candidates_extraction/config.py`.
