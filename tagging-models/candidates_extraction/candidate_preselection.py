...
def extract_candidates(args: argparse.Namespace) -> Path:
    ...
    if args.root is not None:
        # A CLI ROOT path remains useful for one-off single-file extractions.
        if args.prompt_vertex_cm is None:
            raise ValueError("--prompt-vertex-cm is required when --root is used")
        input_samples = [
            {
                "path": Path(args.root),
                "prompt_vertex_cm": args.prompt_vertex_cm,
            }
        ]
    else:
        if args.prompt_vertex_cm is not None:
            raise ValueError("--prompt-vertex-cm can only be used together with --root")
        input_samples = INPUT_SAMPLES
    if not input_samples:
        raise ValueError("candidates_extraction/config.py must contain at least one input sample")
    ...
    for source_file_id, sample in enumerate(input_samples):
        root_path = Path(sample["path"])
        prompt_vertex_cm = np.asarray(sample["prompt_vertex_cm"], dtype=float)
        if prompt_vertex_cm.shape != (3,) or not np.all(np.isfinite(prompt_vertex_cm)):
            raise ValueError(f"Invalid prompt_vertex_cm for {root_path}: {prompt_vertex_cm}")

        # ROOT entry numbers restart in each file, so candidate identities are
        # de-duplicated independently for every input sample.
        seen_candidate_hitsets = set()
        seen_raw_candidate_hitsets = set()

        with uproot.open(root_path) as root_file:
            wtree = root_file["WCTEReadoutWindows"]
            ttree = root_file["TTrueInfo"]
            ...
            missing_w = [branch for branch in required_w if branch not in wtree_keys]
            if missing_w:
                raise RuntimeError(
                    f"Missing WCTE branches in {root_path}: " + ", ".join(missing_w)
                )

            w_branches = required_w + (
                ["event_number"] if "event_number" in wtree_keys else []
            )
            optional_w = ["hit_mpmt_card_ids", "hit_pmt_channel_ids"]
            w_branches += [branch for branch in optional_w if branch in wtree_keys]
            ...
            t_branches = [branch for branch in optional_t if branch in ttree_keys]
            ...
            for start in range(0, n_entries, args.chunk_size):
                ...
                for i in range(stop - start):
                    ...
                    for row in evt_rows:
                        ...
                        row["source_file"] = str(root_path)
                        row["source_entry"] = source_entry
                    for row in evt_root_rows:
                        row["source_file_id"] = source_file_id
                        row["source_entry"] = source_entry
                    rows.extend(evt_rows)
                    root_rows.extend(evt_root_rows)
                    for key, value in evt_counts.items():
                        counters[key] = counters.get(key, 0) + int(value)

                print(
                    f"Processed {root_path.name}: entries {start:,}-{stop:,} / "
                    f"{n_entries:,}; capture_candidates so far = {len(rows):,}",
                    flush=True,
                )
    ...
    summary = {
        "input_samples": sample_summaries,
        ...
        "root_hit_info": {
            ...
            "source_branches": ["source_file_id", "source_entry"],
            "label_branch": "label",
...
def add_extract_args(p: argparse.ArgumentParser) -> None:
    ...
    p.add_argument(
        "--root",
        default=None,
        help="Optional single ROOT file override; otherwise use INPUT_SAMPLES from candidates_extraction/config.py",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=0,
        help="Maximum entries per input file; 0 means all entries",
    )
    ...
    p.add_argument(
        ...
        nargs=3,
        default=None,
        help="Required source position in cm for a single --root override",
    )
    p.add_argument(
        "--bonsai-dir",
