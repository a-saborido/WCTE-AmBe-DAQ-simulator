...
- `config.py`: input ROOT/source-position pairs, tunable values, and observable groups.
...
All extraction options are attached in `add_extract_args()`. The default ROOT
files and their prompt/source positions are paired in `INPUT_SAMPLES` in
`config.py`. Positions are stored in cm. Timing windows, double-grid
multilateration settings, angular cuts, Nn/Nmax200 cuts, and truth-label
thresholds are defined in the same file.
...
The default command processes every configured input sample and writes all
candidates to one table. `--root FILE --prompt-vertex-cm X Y Z` remains
available for a single-file override.
...
It then opens each configured ROOT file and uses that file's
`prompt_vertex_cm`. The file-specific position is used for prompt TOF
correction, the local vertex scan, and `fpdist`.
...
`extract_candidates()` opens each configured input file with `uproot` and reads:
...
calculation uses PMT geometry and the source position configured for the current
ROOT file; it does not use truth information.
...
Both tracking sets are shared by all `event_to_candidates()` calls for one ROOT
file. They are reset before the next file because ROOT entry numbers restart in
each sample.
...
- `source_file`, `source_file_id`: configured ROOT input and its numeric id.
- `source_entry`: anchor readout-window entry inside that file. Training uses
  `(source_file, source_entry)` to keep each window in one split.
...
jagged arrays; scalar branches store the source file id and entry, candidate
label, fitted vertex, `fpdist`, prompt time, and capture-hit counts. This layout
can be used to train point-cloud methods.
...
- `nn_cut`
- whether each input ROOT file uses the expected time and charge branches
